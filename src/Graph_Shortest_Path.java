...
class Graph_Shortest_Path { 
    ...
    int minDistance(FlightInfoList path_array[], Boolean sptSet[])   { 
        // Initialize min value 
        // int min = Integer.MAX_VALUE, min_index = -1; 
        int cheapestCost = Integer.MAX_VALUE;
        for (int v = 0; v < numCities; v++) {
        
            for (FlightInfo flight : path_array[v].getFlightInfoList()) {
                if (flight.getPrice() < cheapestCost) {
                    cheapestCost = flight.getPrice();
                }
            }

            // TO BE REVIEWED
            if (sptSet[v] == false && path_array[v].getFlightInfoList() <= cheapestCost) { 
                cheapestCost = path_array[v]; 
                min_index = v; 
            } 
...
// Implementation of Dijkstra's algorithm for graph (adjacency matrix) 
    void algo_dijkstra(int graph[][], int src_node)  { 
        int path_array[] = new int[numCities]; // The output array. dist[i] will hold 
        // the shortest distance from src to i 
   
        // spt (shortest path set) contains vertices that have shortest path 
        Boolean sptSet[] = new Boolean[numCities]; 
